/** WebUI application entry (ES module). */
import { createApi } from "./api.js";
import { $, durationLabel, formatTime, setValue, switchTab, toast } from "./dom.js";
import {
  applyRunDraft,
  bindRunDraftAutosave,
  clearRunDraft,
  loadRunDraft,
  saveRunDraft,
} from "./run_draft.js";
import { readTokenFromUrl } from "./storage.js";
...
function setRunState(status) {
  ...
  if ($("accountCount")) $("accountCount").textContent = String(status.accounts?.total ?? 0);
  if ($("accountsPath")) $("accountsPath").textContent = status.accounts?.path || "accounts_cli.txt";
  if ($("cpaCount")) $("cpaCount").textContent = String(status.cpa?.count ?? 0);
  if ($("cpaPath")) $("cpaPath").textContent = status.cpa?.path || "cpa_auths";
  if ($("mailCount")) $("mailCount").textContent = String(status.mailbox?.count ?? 0);
  if ($("mailPath")) $("mailPath").textContent = status.mailbox?.path || "mail_credentials.txt";
  if ($("processId")) $("processId").textContent = status.pid ? String(status.pid) : "—";
  if ($("runDuration")) {
    $("runDuration").textContent = durationLabel(status.started_at, status.running, status.ended_at);
  }

  if ($("startButton")) $("startButton").disabled = status.running;
  if ($("stopButton")) $("stopButton").disabled = !status.running;
  if ($("saveConfigButton")) $("saveConfigButton").disabled = status.running;
  if ($("lastExitState")) {
...
function renderReadiness(status) {
  ...
  if (
    (method === "browser" && ["tempmail", "tempmail.lol"].includes(provider) && !secrets.tempmail_api_key) ||
    (method === "protocol" && ["tempmail", "tempmail.lol"].includes(provider) && !secrets.tempmail_api_key)
  ) {
    warnings.push("Tempmail API Key 未配置");
  }
...
function updateProviderFields() {
  const method = $("registrationMethodConfig")?.value || "browser";
...
function fillPresetValues(preset) {
  ...
  setValue("tempmailBaseConfig", values.tempmail_base_url || "https://api.tempmail.lol");
  setValue("tempmailPrefixConfig", values.tempmail_prefix || "grok");
  setValue("tempmailProxyConfig", values.tempmail_proxy || "");
  if ($("cpaManagementKeyConfig")) {
    $("cpaManagementKeyConfig").placeholder = secrets.cpa_management_key
      ? "已配置，留空保持现有值"
      : "未配置";
  }
  if ($("protocolMoeMailKeyConfig")) {
    $("protocolMoeMailKeyConfig").placeholder = secrets.protocol_moemail_api_key
      ? "已配置，留空保持现有值"
      : "未配置";
  }
  if ($("protocolYesCaptchaKeyConfig")) {
  ...
  for (const [id, key] of [
    ...
    ["tempmailKeyConfig", "tempmail_api_key"],
...
function updateRunPreset({ keepDraft = true, persistDraft = true } = {}) {
  ...
  if ($("presetSummary")) $("presetSummary").textContent = `${method} · ${provider}`;

  // Preset defaults first…
  if ($("threadsInput")) $("threadsInput").value = String(preset.values.register_threads ?? 1);
  if ($("mintWorkersInput")) $("mintWorkersInput").value = String(preset.values.cpa_mint_workers ?? -1);
  const isOutlook = ["outlook", "hotmail"].includes(provider);
  if ($("aliasRunSection")) $("aliasRunSection").hidden = !isOutlook;
  if ($("aliasEnabledInput")) {
    $("aliasEnabledInput").checked =
      isOutlook && (preset.values.hotmail_alias_mode || "primary") !== "primary";
  }
  if ($("aliasLimitInput")) {
    $("aliasLimitInput").value = String(preset.values.hotmail_max_aliases_per_account ?? 1);
  }

  // …then re-apply remembered temporary overrides for the run panel.
  if (keepDraft) {
    const draft = loadRunDraft();
    if (draft && (!draft.preset_id || draft.preset_id === preset.id || !state.draftApplied)) {
      applyRunDraft(draft, { preferPreset: false });
...
function updateAliasControls() {
  const enabled = Boolean($("aliasEnabledInput")?.checked);
  if ($("aliasLimitField")) $("aliasLimitField").hidden = !enabled;
  refreshMailInventory();
}

function renderMailInventory(inventory) {
  const aliasMode = inventory.mode === "alias";
  if ($("mailInventorySummary")) {
...
async function refreshMailInventory() {
  ...
  try {
    ...
    if ($("mailInventorySummary")) $("mailInventorySummary").textContent = error.message;
  }
}
...
function fillConfig(config) {
  ...
  const active =
    getPreset(config.active_preset_id) ||
    config.presets?.[0] || {
      id: "",
      name: "默认配置",
  ...
  updateRunPreset({ keepDraft: true });
...
function readConfigForm() {
  ...
  const registrationMethod = $("registrationMethodConfig")?.value || "browser";
  const provider = $("emailProviderConfig")?.value || "outlook";
  ...
  return {
    ...
    values: {
      ...
      tempmail_api_key: $("tempmailKeyConfig")?.value || "",
      tempmail_base_url: ($("tempmailBaseConfig")?.value || "").trim(),
      tempmail_prefix: ($("tempmailPrefixConfig")?.value || "").trim(),
      tempmail_proxy: ($("tempmailProxyConfig")?.value || "").trim(),
...
async function saveConfig(event) {
  ...
  try {
    ...
    if ($("cpaManagementKeyConfig")) $("cpaManagementKeyConfig").value = "";
    if ($("clearManagementKeyConfig")) $("clearManagementKeyConfig").checked = false;
    for (const id of [
    ...
    if ($("clearProtocolSecretsConfig")) $("clearProtocolSecretsConfig").checked = false;
    fillConfig(config);
    toast("配置已保存", "success");
    await refreshStatus();
  } catch (error) {
    ...
    if ($("saveConfigButton")) $("saveConfigButton").disabled = state.running;
  }
}

function newPreset() {
  state.editingPresetId = `preset_${Date.now().toString(36)}`;
  if ($("presetNameConfig")) $("presetNameConfig").value = "新预设";
  if ($("presetSelectConfig")) $("presetSelectConfig").value = "";
  toast("编辑完成后保存新预设");
}
...
function bindEvents() {
  $("runPresetInput")?.addEventListener("change", () => updateRunPreset({ keepDraft: false }));
  ...
  $("rememberDraftButton")?.addEventListener("click", () => saveRunDraft(false));
...
async function initialize() {
  ...
  // After presets load, restore remembered temporary run settings.
  const draft = loadRunDraft();
  if (draft) {
    applyRunDraft(draft, { preferPreset: true });
    state.draftApplied = true;
    updateRunPreset({ keepDraft: true });
    toast("已恢复上次的临时运行配置");
  }
  ...
  window.setInterval(() => {
    if (state.status && $("runDuration")) {
      $("runDuration").textContent = durationLabel(
        state.status.started_at,
        state.status.running,
        state.status.ended_at,
      );
    }
  }, 1000);
}
