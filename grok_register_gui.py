...
def fill_code_and_submit(email, dev_token, timeout=180, log_callback=None, cancel_callback=None):
    ...
    if page_on_signup_chooser(page):
        raise Exception(f"未进入验证码页（仍在「使用邮箱注册」方式页）: {email}")
    ...
    if not page_has_code_input(page):
        ...
        for _ in range(16):
            ...
            if not page_has_code_input(page) and not has_profile_form(log_callback=None):
                take_screenshot(page, "no-code-page")
...
return false;
    ...
    while time.time() < deadline:
        raise_if_cancelled(cancel_callback)
        page = sync_active_page() or _get_page()
        dismiss_cookie_banner(page, log_callback=None)
        ...
        if has_profile_form(log_callback=None):
            ...
            return code
        if page_on_signup_chooser(page):
            take_screenshot(page, "code-phase-chooser")
            raise Exception(f"验证码阶段页面回到注册方式页（使用邮箱注册）: {email}")
        if page_still_on_email_form(page) and not page_has_code_input(page):
            take_screenshot(page, "code-phase-email")
            raise Exception(f"验证码阶段退回邮箱表单: {email}")
        if not page_has_code_input(page):
            human_sleep(0.5, cancel_callback)
            continue

        # 1) real keystrokes first (GUI-only hardening)
...
return 'not-ready';
        ...
        human_sleep(0.35, cancel_callback)
        clicked = page.run_js(
            r"""
function isVisible(node) {
    if (!node) return false;
...
return 'no-button';
        ...
        # DP native click as extra submit path
        if clicked in (None, "", "no-button"):
            for text in ("确认邮箱", "继续", "下一步", "验证", "Continue", "Verify", "Next"):
                try:
                    btn = page.ele(f"text:{text}", timeout=0.4)
                    if btn:
                        btn.click()
                        clicked = f"dp-click:{text}"
                        break
                except Exception:
                    pass
        ...
        submit_tries += 1
        if log_callback:
            log_callback(f"[*] 已填写验证码并提交: {code} ({filled}/{clicked}, 第{submit_tries}次)")
        dump_state(page, "code-submitted")
        take_screenshot(page, "code-submitted")

        # MUST reach profile (or hard fail). Never treat bounce-to-chooser as success.
        confirm_secs = float(config.get("code_form_timeout", 45) or 45)
        # cap single confirm wait so multi-submit retries still fit overall timeout
        confirm_secs = max(12.0, min(confirm_secs, 45.0))
        confirm_deadline = time.time() + confirm_secs
...
- 如果关闭了启动教程，可随时点主界面的“教程”按钮重新打开。"""
    ...
    def _run_single_registration(self, idx, total, logf):
        ...
        mail_ok = False
        max_mail_retry = 5
        # Align with CLI: new Resin sticky session + clean browser per account.
        try:
            stop_browser()
        except Exception:
            pass
        proxy_account = begin_registration_proxy_session(f"gui_n{idx}")
        if proxy_account:
